...
def audit_cloudtrail(
    ...
    try:
        ...
        if not warn_permission_error('aws-read', e):
            logger.warning("Error checking CloudTrail in %s: %s", region, e)
...
def audit_cloudwatch_log_groups(
    ...
    try:
        ...
        if not warn_permission_error('aws-read', e):
            logger.warning("Error checking CloudWatch Log Groups in %s: %s", region, e)
...
def audit_vpc_flow_logs(
    ...
    try:
        ...
        if not warn_permission_error('aws-read', e):
            logger.warning("Error checking VPC Flow Logs in %s: %s", region, e)
