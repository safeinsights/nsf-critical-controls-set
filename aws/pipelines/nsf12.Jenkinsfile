pipeline {
    ...
    stages {
        ...
        stage('NSF12: Vulnerability Management') {
            steps {
                sh '''
                    cd aws
                    . venv/bin/activate
                    mkdir -p ${WORKSPACE}/output
                    python3 -m audits.nsf12 \
    ...
    post {
        ...
        failure {
            emailext(
                ...
                subject: "NSF12 Audit Failed - Build ${env.BUILD_NUMBER}",
                body: "NSF12 Vulnerability Management audit failed.\n\nCheck ${env.BUILD_URL} for details."
            )
        }
    }
}
